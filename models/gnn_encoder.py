...
from .gnn_layer import StarEConvLayer, StarEConvLayer_gradient
...
class StarEEncoder(StarEBase):
    def __init__(self, graph_repr: Dict[str, np.ndarray], config: dict, timestamps: dict = None):
        ...
        if timestamps is None:
            if config['STAREARGS']['SEP_ENT_EMBEDDING']:
                # each entity has main entity embedding and qualifier entity embedding
                self.init_embed = get_param((self.num_ent, self.emb_dim * 2))
            else:
        ...
        if config['STAREARGS']['ENCODER'] == "StarE":
            conv_fn = StarEConvLayer
        elif config['STAREARGS']['ENCODER'] == "StarE_grad":
            conv_fn = StarEConvLayer_gradient
        else:
            raise NotImplementedError

        self.conv1 = conv_fn(self.emb_dim, self.gcn_dim, self.num_rel, act=self.act,
                                       config=config)
        self.conv2 = conv_fn(self.gcn_dim, self.emb_dim, self.num_rel, act=self.act,
    ...
    def forward_base(self, sub, rel, drop1, drop2,
        ...
        if self.config['STAREARGS']['MASK_EDGE'] and obj is not None:
            edge = torch.cat([self.edge_index, self.edge_type.reshape(1,-1)], dim=0).transpose(0,1)
            edge_to_predict = torch.stack([sub, obj, rel]).transpose(0,1)
            # edge_to_predict_reverse = torch.cat([obj, sub, rel], dim=0)
            # edge_to_predict = torch.cat(edge_to_predict)
            mask = edge.unsqueeze(1).eq(edge_to_predict).all(-1).any(-1)
            edge[mask] = 0
            edge = edge.transpose(0,1)
            edge_index = edge[:2]
            edge_type = edge[-1]
            quals_graph = self.quals
        else:
        ...
        if not self.triple_mode:
            if self.qual_mode == "full":
                ...
                x, r = self.conv1(x=self.init_embed, edge_index=edge_index,
                ...
                x = drop1(x)
                x, r = self.conv2(x=x, edge_index=edge_index,
                                  edge_type=edge_type, rel_embed=r,
                                  qualifier_ent=self.qual_ent,
                                  qualifier_rel=self.qual_rel,
                ...
                # x, edge_index, edge_type, rel_embed, qual_ent, qual_rel
                x, r, score = self.conv1(x=self.init_embed, edge_index=edge_index,
                                  edge_type=edge_type, rel_embed=r,
                                  qualifier_ent=None,
                                  qualifier_rel=None,
                                  quals=quals_graph)
                self.score_list = [score[0]]
                x = drop1(x)
                x, r, score = self.conv2(x=x, edge_index=edge_index,
                                  edge_type=edge_type, rel_embed=r,
                                  qualifier_ent=None,
                                  qualifier_rel=None,
                                  quals=quals_graph) if self.n_layer == 2 else (x, r)
                self.score_list.append(score[0])
        ...
        if self.config['STAREARGS']['SEP_ENT_EMBEDDING']:
            sub_emb = torch.index_select(x[:, :x.shape[1]//2], 0, sub)
        else:
        ...
        if embed_qualifiers:
            ...
            if self.config['STAREARGS']['SEP_ENT_EMBEDDING']:
                qual_obj_emb = torch.index_select(x[:, x.shape[1]//2:], 0, quals_ents)
            else:
