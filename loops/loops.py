    ...
    for e in range(epochs):
        ...
        with Timer() as timer:
            ...
            for batch in tqdm(trn_dl):
                ...
                if "mask" in train_fn.model_name:
                ...
                if qualifier_aware:
                    pred = train_fn(_sub, _rel, _quals, _obj)
                else:
                ...
                belief_score.append(train_fn.score_list)

                loss.backward()
                # assert not torch.isnan(train_fn.init_embed.grad).any()
        ...
        # Log this stuff
        print(f"[Epoch: {e} ] Loss: {np.mean(per_epoch_loss)} LR: {scheduler._last_lr[0]}")
        ...
        if e % eval_every == 0 and e >= 1:
            with torch.no_grad():
                ...
                if run_trn_testbench:
                    ...
                    if log_wandb:
                        ...
                        wandb.log({
                            'epoch': e,
                            'loss': float(np.mean(per_epoch_loss)),
                            'score_1': float(np.mean([score[0] for score in belief_score])),
                            'score_2': float(np.mean([score[1] for score in belief_score])),
                            'val_acc': float(per_epoch_vl_acc),
                            'val_mrr': float(per_epoch_vl_mrr),
                            'val_mr': float(per_epoch_vl_mr),
                            'val_hits@3': float(per_epoch_vl_hits_3),
                ...
                if savedir is not None:
                    mt_save(
                        savedir,
                        torch_stuff=[tosave(obj=save_content['model'].state_dict(), fname='model.pt')],
